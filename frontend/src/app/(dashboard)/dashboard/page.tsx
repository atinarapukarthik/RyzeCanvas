...
export default function DashboardPage() {
  ...
  const handlePlanFirst = () => {
    router.push(`/studio?prompt=${encodeURIComponent(prompt)}&mode=plan`);
  };

  const handleBuildDirectly = () => {
    router.push(`/studio?prompt=${encodeURIComponent(prompt)}&mode=build`);
  };
          ...
          >
            {[
              ...
              <button
                ...
                onClick={() => handleSend(`Build a ${suggestion.label.toLowerCase()}`)}
            ...
            {showActionChoice && (
              ...
              >
                <motion.div
                  ...
                  className="bg-[hsl(228,12%,7%)] border border-white/10 rounded-2xl p-6 max-w-lg w-full mx-4 shadow-2xl"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="mb-5">
                    <h3 className="text-lg font-semibold text-white mb-1">How would you like to proceed?</h3>
                    <p className="text-sm text-white/40 line-clamp-2">&quot;{prompt}&quot;</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <motion.button
                      ...
                      className="group relative flex flex-col items-start gap-3 p-5 rounded-xl bg-white/5 border border-white/10 hover:border-[hsl(280,72%,68%)]/50 hover:bg-[hsl(280,72%,68%)]/5 transition-all duration-200 text-left"
                    >
                      <div className="flex items-center justify-center h-10 w-10 rounded-lg bg-[hsl(280,72%,68%)]/10 text-[hsl(280,72%,68%)]">
                        <Lightbulb className="h-5 w-5" />
                      </div>
                      <div>
                        <h4 className="font-medium text-white text-sm">Plan First</h4>
                        <p className="text-xs text-white/40 mt-1">
                          Review an AI-generated plan before building
                        </p>
                      </div>
                      <ArrowRight className="absolute top-5 right-4 h-4 w-4 text-white/20 group-hover:text-[hsl(280,72%,68%)] transition-colors" />
                    </motion.button>

                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={handleBuildDirectly}
                      className="group relative flex flex-col items-start gap-3 p-5 rounded-xl bg-white/5 border border-white/10 hover:border-[hsl(234,89%,74%)]/50 hover:bg-[hsl(234,89%,74%)]/5 transition-all duration-200 text-left"
                    >
                      <div className="flex items-center justify-center h-10 w-10 rounded-lg bg-[hsl(234,89%,74%)]/10 text-[hsl(234,89%,74%)]">
                        <Zap className="h-5 w-5" />
                      </div>
                      <div>
                        <h4 className="font-medium text-white text-sm">Build Directly</h4>
                        <p className="text-xs text-white/40 mt-1">
                          Skip planning and start generating code now
                        </p>
                      </div>
                      <ArrowRight className="absolute top-5 right-4 h-4 w-4 text-white/20 group-hover:text-[hsl(234,89%,74%)] transition-colors" />
                    </motion.button>
                  </div>

                  <button
                    onClick={handleDismiss}
                    className="mt-4 w-full text-center text-xs text-white/30 hover:text-white/50 transition-colors py-2"
                  >
                    Cancel
                  </button>
                </motion.div>
